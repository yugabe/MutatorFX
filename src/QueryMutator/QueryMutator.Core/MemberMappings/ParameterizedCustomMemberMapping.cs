...
namespace MutatorFX.QueryMutator.MemberMappings
{
    ...
    {
        public ParameterizedCustomMemberMapping(ParameterExpression sourceParameter, Expression<Func<TTarget, TMember>> memberSelector, Func<TParameter, Expression<Func<TSource, TMember>>> mappingExpressionFactory) 
            : base(sourceParameter, (memberSelector.Body as MemberExpression).Member)
        {
        ...
        {
            return ExpressionFactory(parameter).ReplaceParameter(SourceParameter).Body;
        }
    }
}
